...
for i = 2:nrows-1
    for j = 2:ncols-1
        if ground(i,j)==1
            ...
            if contact_edges(i,j)==1 % Point on contact edge
                % dY/dy = 0
                c = c+1;   % increment constraint counter
            ...
            if vericalsum>0 && groundsum==0 % Point on or near vertical edge
                % dY/dy = 1/cos a
                c = c+1; % increment constraint counter
            ...
            if horizontalsum>0 && groundsum==0 && vericalsum==0 % Point on or near horizontal edge
                % dY/dx = 0
                c = c+1; % increment constraint counter
                Aij(:,:,c) = - (dy / sqrt(dx^2 + dy^2)) * [-1 -2 -1; 0 0 0; 1 2 1]' + (dx / sqrt(dx^2 + dy^2)) * [-1 -2 -1; 0 0 0; 1 2 1]; % !!!! FILL MISSING KERNEL HERE;
                ii(c) = i; 
                jj(c) = j;
                b(c)  = 0;
            end
            if groundsum==0 % Point on object surface
                % laplacian = 0
                c = c+1; % increment constraint counter 
